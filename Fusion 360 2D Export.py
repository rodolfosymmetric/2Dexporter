...
class TwoDExport(object):
    ...
    def _show_configuration_dialog(self):
        ...
        try:
            ...
            # Check if format was already selected via HTML interface
            if hasattr(self, 'selected_export_format') and self.selected_export_format:
                # Format already selected in HTML - use it
                if self.selected_export_format.lower() == 'pdf':
                    self.export_formats = ['pdf']
                elif self.selected_export_format.lower() == 'dxf':
                    self.export_formats = ['dxf']
                else:
                    # Fallback to PDF if unknown format
                    self.export_formats = ['pdf']
            else:
                # Step 2: Export format selection (fallback for non-HTML usage)
    ...
    def _export_selected_folders(self, output_path, progress_dialog):
        ...
        # Use the selected folder for export
        if hasattr(self, 'selected_folder_result') and self.selected_folder_result:
            selected_folder = self.selected_folder_result
            target_path = getattr(self, 'target_export_path', None)
            
            if not target_path:
                self.ui.messageBox("No target export folder was selected")
                return
                
            self.ui.messageBox("Exporting from folder: {} to: {}".format(selected_folder.name, target_path))
        else:
            self.ui.messageBox("No source folder was selected")
            return

        all_files = []
        
        # Collect files from the selected folder (recursive)
        self.log.info("Processing selected folder \"{}\"".format(selected_folder.name))
        all_files = self._get_files_for(selected_folder)
        ...
        if not all_files:
            self.log.info("No files found in selected folder")
            return
        
        self.log.info("Found {} files in selected folder".format(len(all_files)))
        
        # Export each file to the target path
        for i, file in enumerate(all_files):
            ...
            self._export_file(file, target_path)
...
class HTMLEventHandler(adsk.core.HTMLEventHandler):
    ...
    def notify(self, args):
        try:
            ...
            if action == 'getHubs':
                self._send_hubs_to_html(htmlArgs)
            elif action == 'getProjects':
                hub_index = int(data)
                self._send_projects_to_html(self.exporter.current_palette, hub_index)
            elif action == 'getFolders':
                project_index = int(data)
                self._send_folders_to_html(self.exporter.current_palette, project_index)
            elif action == 'browseTargetFolder':
                self._browse_target_folder()
            elif action == 'startExport':
                self._handle_export_start(data)
            elif action == 'cancel':
                self.exporter.cancelled = True
                if hasattr(self.exporter, 'current_palette') and self.exporter.current_palette:
                    self.exporter.current_palette.close()
            else:
                self.exporter.log.info("Unknown action: {}".format(action))
                
        except Exception as ex:
            self.exporter.log.error("Error in HTML event handler: {}".format(str(ex)))
    ...
    def _browse_target_folder(self):
        """Open folder browser dialog for target folder selection"""
        try:
            # Use Fusion 360's folder dialog
            folderDialog = self.exporter.ui.createFolderDialog()
            folderDialog.title = "Select Export Destination Folder"
            
            if folderDialog.showDialog() == adsk.core.DialogResults.DialogOK:
                selected_path = folderDialog.folder
                if self.exporter.current_palette:
                    self.exporter.current_palette.sendInfoToHTML('onTargetFolderSelected', selected_path)
                    self.exporter.log.info("Target folder selected: {}".format(selected_path))
            
        except Exception as ex:
            self.exporter.log.error("Error browsing target folder: {}".format(str(ex)))
            if self.exporter.current_palette:
                self.exporter.current_palette.sendInfoToHTML('onError', 'Error selecting target folder: {}'.format(str(ex)))

    def _handle_export_start(self, export_data):
        try:
            # Parse the export data (includes source folder, target path, and format)
            import json
            data = json.loads(export_data)
            source_folder_id = data.get('sourceFolder')
            target_path = data.get('targetPath')
            export_format = data.get('exportFormat')
            
            if not hasattr(self.exporter, 'current_folders') or not source_folder_id or not target_path or not export_format:
                return
                
            # Find the selected source folder
            if source_folder_id == self.exporter.current_folders['root'].id:
            ...
            self.exporter.target_export_path = target_path
            self.exporter.selected_export_format = export_format
            self.exporter.palette_closed = True
            
            # Close the palette and start export
            if hasattr(self.exporter, 'current_palette') and self.exporter.current_palette:
                self.exporter.current_palette.close()
            
        except Exception as ex:
            self.exporter.log.error("Error starting export: {}".format(str(ex)))
