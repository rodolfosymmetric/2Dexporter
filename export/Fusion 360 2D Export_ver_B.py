...
class HTMLEventHandler(adsk.core.HTMLEventHandler):
    ...
    def _handle_folder_selection(self, selection_data):
        try:
            # Parse the selection data (now includes both source folder and target path)
            import json
            data = json.loads(selection_data)
            source_folder_id = data.get('sourceFolder')
            target_path = data.get('targetPath')
            
            if not hasattr(self.exporter, 'current_folders') or not source_folder_id or not target_path:
                return
            ...
            # Store the target path for export
